...
3. The user may change tone, give an instruction to AI, replace the text directly, regenerate, cancel, or approve.
   ...
   An instruction may be typed or sent as Telegram Voice. Voice is transcribed concurrently by the configured ASR models. AI reconciles only evidence-supported wording, then confidence, uncertain terms, and optionally both raw outputs are shown for approval or correction before any draft is generated.
